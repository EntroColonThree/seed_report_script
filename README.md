...
- `stats_analysis.py` и `tests/` — описательная статистика, ANOVA/Краскела-Уоллиса, post-hoc, CLD-буквы.
...
Внутри файла — по одной строке на семя: длина колиоптеля (надземной части) и *единственного* корня. Пустая строка или текст просто игнорируется. Количество непроросших задаётся либо строками с одним 0, либо записью 0=N
